...
RCLBCache(pdist::ParticleDistribution{T}, sdist::SplineDistribution{T}) where {T} = RCLBCache{T}(pdist, sdist)

Cache(AT, c::RCLBCache{DT, PT, ST}) where {DT, PT, ST} = RCLBCache{AT}(c.pdist, similar(AT, c.sdist))
CacheType(AT, c::RCLBCache{DT, PT, ST}) where {DT, PT, ST} = RCLBCache{AT, PT, similar_type(AT, c.sdist)}

struct RescaledConservativeLenardBernstein{XD, VD, DT <: DistributionFunction{XD,VD}, ET <: Entropy, T, CT <: CacheDict} <: CollisionOperator
    ...
    function RescaledConservativeLenardBernstein(dist::DistributionFunction{XD,VD}, ent::Entropy; ν::T=1.) where {XD, VD, T}
        cache = CacheDict(RCLBCache(dist, ent.dist))
        new{XD, VD, typeof(dist), typeof(ent), T, typeof(cache)}(dist, ent, ν, cache)
    end
end

Cache(AT, clb::RescaledConservativeLenardBernstein) = RCLBCache{AT}(clb.dist, similar(AT,clb.ent.dist))
CacheType(AT, clb::RescaledConservativeLenardBernstein) = RCLBCache{AT, typeof(clb.dist), similar_type(AT, clb.ent.dist)}

function compute_coefficients_rclb(distribution::SplineDistribution, particle_dist::ParticleDistribution, vp::AbstractArray{VT}) where {VT}
