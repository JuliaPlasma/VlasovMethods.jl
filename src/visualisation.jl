...
function plot_distribution(
        f::Function, xmin::T, xmax::T, vmin::T, vmax::T; nx = 100, nv = 100) where {T}
    xGrid = collect(LinRange(xmin, xmax, nx))
    vGrid = collect(LinRange(vmin, vmax, nv))
    plot_distribution(f, xGrid, vGrid)
end

function plot_distribution(X::AbstractVector, V::AbstractVector, W::AbstractVector,
        xmin, xmax, vmin, vmax; nx = 100, nv = 100)
    xGrid = collect(LinRange(xmin, xmax, nx))
    vGrid = collect(LinRange(vmin, vmax, nv))
...
function plot_distribution(
        x::AbstractMatrix, v::AbstractMatrix, w::AbstractMatrix, args...; kwargs...)
    @assert size(x, 1) == 1
    @assert size(v, 1) == 1
    @assert size(w, 1) == 1
...
function plot_particles(
        X::AbstractVector, V::AbstractVector, W::AbstractVector, xmin, xmax, vmin, vmax)
...
function plot_particles(X::AbstractVector, V::AbstractVector, W::AbstractVector,
        xGrid::AbstractVector, vGrid::AbstractVector)
