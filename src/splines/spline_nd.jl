...
function remap_unit_to_knot_interval(f::Callable, knots, k, l)
    v -> f(SA_F64[remap_unit_interval(v[1], knots[k], knots[k + 1]), remap_unit_interval(
        v[2], knots[l], knots[l + 1])])
end
...
function remap_basis_from_unit_to_knot_interval(i::Int, basis::PeriodicBSplineBasis, knots, k)
    v -> basis[i](remap_unit_interval(v, knots[k], knots[k + 1])) +
         basis[i](remap_unit_interval(v, knots[k], knots[k + 1]) -
                  (knots[end] - knots[begin])) +
         basis[i](remap_unit_interval(v, knots[k], knots[k + 1]) +
                  (knots[end] - knots[begin]))
end

unique_knots(basis::AbstractBSplineBasis) = sort([Set(BSplineKit.knots(basis))...])
function unique_knots(basis::PeriodicBSplineBasis)
    sort([Set(BSplineKit.knots(basis))...])[(begin + div(BSplineKit.order(basis), 2)):(end - div(
        BSplineKit.order(basis) - 1, 2))]
end

function mass_matrix_quadrature(
        basis::AbstractBSplineBasis, knots, quadrature::QuadratureRule, i, j)
    m = 0.0
...
function spline_basis(p::BSplineOrder, knots::AbstractVector, ::Val{:Natural})
    BSplineBasis(p, knots)
end

function spline_basis(p::BSplineOrder, knots::AbstractVector, ::Val{:Dirichlet})
    RecombinedBSplineBasis(BSplineKit.Derivative(0), BSplineBasis(p, knots))
end

function spline_basis(p::BSplineOrder, knots::AbstractVector, ::Val{:Periodic})
...
struct SplineND{T, D, BT <: AbstractBSplineBasis, DT, CT1 <: AbstractArray{T},
    CT2 <: AbstractVector{T}, MT <: AbstractMatrix{T}, FT, QT <: QuadratureRule{T}}
    basis::BT
    derivative::DT
    coefficients::CT1
    coeff_vector::CT2
    ...
    function SplineND{T, D}(basis::AbstractBSplineBasis, quadrature::QuadratureRule;
            mass_quadrature = quadrature) where {T, D}
...
SplineND(ndims::Int, args...; kwargs...) = SplineND{Float64, ndims}(args...; kwargs...)

function SplineND(
        ndims::Int, o::Int, knots::AbstractVector, bcs::Symbol, args...; kwargs...)
    SplineND(ndims, spline_basis(BSplineOrder(o), copy(knots), Val(bcs)), args...; kwargs...)
end

function SplineND(ndims::Int, o::Int, knots::AbstractVector, args...; kwargs...)
...
function _evaluate_basis(B::AbstractBSplineBasis{O, T}, x::AbstractVector{T}, I::Tuple) where {
        O, T}
    result = one(T)
...
function _evaluate_basis_derivative(
        B::AbstractBSplineBasis{O, T}, x::AbstractVector{T}, I::Tuple, j::Int) where {O, T}
    result = one(T)
    for d in eachindex(x)
...
function evaluate_basis_derivative(s::SplineND{T, D}, x::AbstractVector{T}, I::Tuple) where {
        T, D}
...
function evaluate_indices(s::SplineND{T, D}, x::Vararg{T, D}) where {T, D}
    evaluate_indices(s, SVector{D}(x...))
end

function L2product_quadrature(
        f::Callable, basis::AbstractBSplineBasis, knots, quadrature::QuadratureRule, i)
    m = 0.0
...
function L2product_quadrature(
        f::Callable, basis::AbstractBSplineBasis, knots, quadrature::QuadratureRule, i, j)
    m = 0.0
    ...
    for k in eachindex(knots[begin:(end - 1)])
        for l in eachindex(knots[begin:(end - 1)])
            ...
            quadfunc = (i, j) -> quadrature.weights[i] * quadrature.weights[j] *
                                 integrand(quadrature.nodes[i], quadrature.nodes[j])
            quadinds = zip(eachindex(quadrature) * ones(Int, nnodes(quadrature))',
                ones(Int, nnodes(quadrature)) * eachindex(quadrature)')
...
function L2projection!(f::Callable, s::SplineND{T}) where {T}
    L = [L2product_quadrature(
             f, basis(s), unique_knots(s), s.quadrature, cartesian_index(s, l)...)
         for l in 1:length(s)]
