...
struct BumpOnTail{T} <: VlasovExample
    ...
    function BumpOnTail(;
            ε::T = 0.03, κ::T = 0.3, α::T = 0.1, σ::T = 0.5, v₀::T = 4.5) where {T}
        new{T}(ε, κ, α, σ, v₀)
    end
end
...
function initialize!(dist::ParticleDistribution, params::BumpOnTail,
        sampling::SamplingMethod = AcceptRejectSampling())
