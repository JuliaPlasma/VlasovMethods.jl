...
run_name = "double_maxwellian_N=1000_T=2000_new"
...
tstep = 0.8    # time step size
tspan = (0.0, 2)    # integration time interval
domainv = (-10., 10.)
domainv_p = (-2., 2.)
...
# @show step = size(z,2) ÷ nplot
# ent = zeros(nplot)
# dSdt = zeros(nplot, size(z,1))
# v̇ = zeros(nplot, size(z,1))
# # create animations
# anim = @animate for (i, n) in zip(1:nplot, 1:step:size(z,2))
#     println(i)
